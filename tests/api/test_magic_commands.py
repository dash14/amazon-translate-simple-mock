...
class TestReturnSourceLanguageCode:
    def test_overrides_source_language_code(self, post, text_payload):
        response = post(
            text_payload("@return SourceLanguageCode fr", source="ja", target="en")
        )
        assert response.status_code == 200
    ...
    def test_overrides_auto_detection(self, post, text_payload):
        response = post(
            text_payload("@return SourceLanguageCode de", source="auto", target="en")
        )
        assert response.status_code == 200
...
class TestRaiseCommands:
    def test_raise_throttling_exception(self, post, text_payload):
        response = post(
            text_payload("@raise ThrottlingException", source="ja", target="en")
        )
    ...
    def test_raise_internal_server_exception(self, post, text_payload):
        response = post(
            text_payload("@raise InternalServerException", source="ja", target="en")
        )
    ...
    def test_raise_limit_exceeded_exception(self, post, text_payload):
        response = post(
            text_payload("@raise LimitExceededException", source="ja", target="en")
        )
    ...
    def test_raise_service_unavailable_exception(self, post, text_payload):
        response = post(
            text_payload("@raise ServiceUnavailableException", source="ja", target="en")
        )
        assert response.status_code == 500
    ...
    def test_raise_too_many_requests_exception(self, post, text_payload):
        response = post(
            text_payload("@raise TooManyRequestsException", source="ja", target="en")
        )
        assert response.status_code == 400
    ...
    def test_raise_unsupported_language_pair_exception(self, post, text_payload):
        response = post(
            text_payload(
                "@raise UnsupportedLanguagePairException", source="ja", target="en"
            )
        )
        assert response.status_code == 400
...
class TestSleepCommand:
    ...
    def test_sleep_with_raise_still_raises(self, post, text_payload):
        response = post(
            text_payload(
                "@sleep 0 @raise ThrottlingException", source="ja", target="en"
            )
        )
        assert response.status_code == 429
