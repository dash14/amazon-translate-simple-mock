...
translators = {"en": en_translator, "ja": ja_translator}


@app.post("/", response_model_by_alias=True, response_model_exclude_none=True)
async def translate(
    request: TranslateRequest, raw_request: Request
) -> TranslateResponse:
    ...
    m = re.search(
        "@return(?:\\s|&nbsp;)+SourceLanguageCode(?:\\s|&nbsp;)+(\\w+)", content
    )
    if m:
    ...
    if source_lang not in lang_codes or target_lang not in lang_codes:
        return JSONResponse(
            content={
                ...
                "message": "Amazon Translate does not support translation from the language of the source text into the requested target language",  # noqa: E501
            },
            status_code=400,
    ...
    if re.search("@return(?:\\s|&nbsp;)+RequestedBody", content):
    ...
    if request.text:
        return TranslateResponse(
            **{
                ...
                "AppliedTerminologies": map(
                    lambda t: {"Name": t}, request.terminology_names
                ),
